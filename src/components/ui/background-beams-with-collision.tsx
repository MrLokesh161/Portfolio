...
const Explosion = ({ ...props }: React.HTMLProps<HTMLDivElement>) => {
  const drops = Array.from({ length: 10 }, (_, index) => ({
    ...
    directionY: Math.floor(Math.random() * -100 - 30),
  }));
  ...
  return (
    <div {...props} className={cn("absolute z-50", props.className)}>
      ...
      {drops.map((drop) => (
        <motion.span
          key={drop.id}
          initial={{ x: drop.initialX, y: 0, opacity: 1 }}
          animate={{
            x: drop.directionX,
            y: drop.directionY,
            opacity: 0,
          ...
          className="absolute h-1 w-1 rounded-full bg-gradient-to-b from-blue-400 to-indigo-600"
        />
      ))}
    </div>
  );
